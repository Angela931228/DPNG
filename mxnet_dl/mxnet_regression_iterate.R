...
rawdata<- read.csv("./ng_58factors_high.csv")
...
sqrt(mean((diff(log(as.vector(acts)))^2)))
sqrt(mean(((log(prs)-log(acts))^2)))
plot(c(1:length(as.vector(prs))), as.vector(prs),col="green",type ="l", ylim=c(1.5,2.4))
lines(c(1:length(as.vector(acts))),as.vector(acts),type='l',col="red")
write.csv(data.frame(prs,acts), "iterate_10high.csv")
...
label_date<-rawdata[c(451:490),1][label_inteval]
